...
import { EmbedBuilder, TextChannel, GuildMember, Client, Message } from "discord.js";
...
async function updateHierarchyEmbed(client: Client) {
    const { channelId, roles } = constants.hierarchy;
    if (!channelId || channelId === "INSERIR_ID_DO_CANAL_DA_HIERARQUIA") return;
    ...
    try {
        ...
        for (const roleConfig of roles) {
            ...
            const roleMembers = members
                .filter((m: GuildMember) => m.roles.cache.has(roleConfig.id) && !assignedMemberIds.has(m.id))
                .sort((a: GuildMember, b: GuildMember) => (a.nickname || a.user.username).localeCompare(b.nickname || b.user.username));

            if (roleMembers.size > 0) {
                hasAnyMember = true;
                
                // Marca os membros como atribuídos
                roleMembers.forEach((m: GuildMember) => assignedMemberIds.add(m.id));

                const memberList = roleMembers.map((m: GuildMember) => `• ${m}`).join("\n");
        ...
        const botMessages = messages.filter((m: Message) => m.author.id === client.user?.id).reverse(); // Ordem cronológica
        ...
        for (let i = 0; i < embeds.length; i++) {
            if (botMessagesArray[i]) {
                await (botMessagesArray[i] as Message).edit({ embeds: [embeds[i]], components: [] });
            } else {
                await (channel as TextChannel).send({ embeds: [embeds[i]] });
            }
        }
        ...
        if (botMessagesArray.length > embeds.length) {
            for (let i = embeds.length; i < botMessagesArray.length; i++) {
                await (botMessagesArray[i] as Message).delete().catch(() => null);
            }
        }
